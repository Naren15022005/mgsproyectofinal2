...
@section('title', 'Lista de Clientes Registrados')
...
<div class="container">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4 header-custom">
        <h1 class="mb-0">Listado de Clientes Registrados</h1>
        <div class="button-group">
            ...
            <a href="{{ route('admin.clientes_registrados.create') }}" class="btn btn-custom-green">
                <i class="fas fa-plus"></i> Agregar Cliente Registrado
            </a>
    ...
    <div id="filterOptions" class="filter-options" style="display: none;">
        <label class="filtrado" for="filterByType">
            <input class="custom-checkbox" type="checkbox" id="filterByType"> Genero
        </label>
        <select id="typeFilter" class="form-control mt-2" style="display: none;">
            <option value="">Todos</option>
            <option value="Ingreso">Masculino</option>
            <option value="Egreso">Femenino</option>
            <option value="Egreso">Otro</option>
        </select>
    ...
    <div class="table-container">
        <table class="table table-striped table-bordered table-custom">
            ...
            <tbody>
                @foreach($clientesRegistrados as $clienteRegistrado)
                <tr>
                    <td>
                        <label class="custom-checkbox">
                    ...
                    <td>{{ $clienteRegistrado->id }}</td>
                    <td>{{ $clienteRegistrado->cliente->nombre ?? 'N/A' }}</td>
                    <td>{{ $clienteRegistrado->user->name ?? 'N/A' }}</td>
                    <td>{{ $clienteRegistrado->tipoMembresia->nombre ?? 'N/A' }}</td>
                    <td>{{ $clienteRegistrado->fecha_inicio }}</td>
                    <td>{{ $clienteRegistrado->duracion }} días</td>
                    <td>{{ $clienteRegistrado->asistencia ? 'Activo' : 'Inactivo' }}</td>
                    <td class="text-center">
                        <a href="{{ route('admin.clientes_registrados.edit', $clienteRegistrado->id) }}" class="btn btn-icon btn-edit">
                        ...
                        <form action="{{ route('admin.clientes_registrados.destroy', $clienteRegistrado->id) }}" method="POST" class="d-inline-block">
                            @csrf
                            @method('DELETE')
